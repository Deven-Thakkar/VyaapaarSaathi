...
// ─── POST /api/download-report — PDF Generation ───
app.post("/api/download-report", async (req, res) => {
  try {
    const { business_id } = req.body;
    
    // Fetch aggregated data from our predict route
    const mlRes = await axios.post(`http://localhost:${PORT}/api/predict`, { business_id });
    const data = mlRes.data;

    if (!data || !data.meta || !data.meta.has_data) {
    ...
    res.setHeader("Content-Disposition", `attachment; filename=VyapaarSaathi_Report_${Date.now()}.pdf`);
    
    doc.pipe(res);

    // Title Section
    doc.fontSize(24).font('Helvetica-Bold').text('VyapaarSaathi Business Report', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(12).font('Helvetica').text(`Date: ${new Date().toLocaleDateString('en-IN')}`, { align: 'center' });
    doc.moveDown(2);
    ...
    const cashflow = data.meta.sales - data.meta.expenses;

    doc.fontSize(12).font('Helvetica');
    doc.text(`Total Sales: Rs. ${data.meta.sales.toLocaleString('en-IN')}`);
    doc.moveDown(0.5);
    doc.text(`Total Expenses: Rs. ${data.meta.expenses.toLocaleString('en-IN')}`);
    doc.moveDown(0.5);
    doc.text(`Net Cashflow: Rs. ${cashflow.toLocaleString('en-IN')}`);
    doc.moveDown(0.5);
    doc.text(`Inventory Value: Rs. ${data.meta.inventory_value.toLocaleString('en-IN')}`);
    doc.moveDown(0.5);
    doc.text(`Udhaar Pending: Rs. ${data.meta.udhaar_given.toLocaleString('en-IN')}`);
    doc.moveDown(2);

    // Insights Section
    doc.fontSize(16).font('Helvetica-Bold').text('AI Business Insights');
    doc.moveTo(50, doc.y + 5).lineTo(550, doc.y + 5).stroke();
    doc.moveDown(1);

    doc.fontSize(12).font('Helvetica');
    if (cashflow < data.meta.expenses) {
      doc.text("! Business may face cash shortage. Reduce expenses or collect udhaar urgently.");
    } else if (cashflow > data.meta.expenses * 1.5) {
      doc.text("V Business is performing exceptionally well.");
    } else {
      doc.text("V Business is stable.");
    }
    
    doc.moveDown(0.5);
    if (data.meta.udhaar_given > data.meta.sales * 0.3) {
      doc.text("! High outstanding payments. Follow up with customers immediately.");
    }

    doc.moveDown(0.5);
    if (data.risk_prediction > 0.6) {
      doc.text("! High financial risk detected by AI model.");
    } else {
      doc.text("V Low to medium financial risk.");
    }

    // Footer
    doc.moveDown(4);
    doc.fontSize(10).fillColor('gray').text('Generated by VyapaarSaathi AI', { align: 'center' });
    doc.text(`Timestamp: ${new Date().toLocaleString('en-IN')}`, { align: 'center' });
